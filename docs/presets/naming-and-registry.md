...
- **`usb-id-map.json`** — the **complete** `usb_id → @model` crosswalk (627 pairs; the small
  integer POD Go reports over USB → the block model). `usb_id` = the index of a model's
  `symbolicID` in POD Go Edit's `PodGo.sym`; generated by `tools/gen_usb_id_map.py`. See
  [reference/usb-id-mapping.md](../reference/usb-id-mapping.md) and rule `usb-id-decode` in
  `rules.json`.
...
| `python3 tools/gen_usb_id_map.py` | Regenerate the **complete** `registry/usb-id-map.json` (627 pairs) from POD Go Edit's `PodGo.sym` (`usb_id` = symbolicID's index; self-checks vs known hardware pairs). See [../reference/usb-id-mapping.md](../reference/usb-id-mapping.md) |
| `python3 tools/decode_model_list.py <capture.log>` | **Superseded** by `gen_usb_id_map.py` (the map is a local file, no capture needed). Kept for reference — recovered the table from a browse-every-category capture. |
